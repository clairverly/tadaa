...
export function Sidebar() {
  ...
  const handleSendMessage = async (messageText?: string) => {
    ...
    try {
      ...
      console.log("AI Extraction Result:", aiResponse);
      setMessages((prev) => [...prev, aiResponse]);
      if (responseText.extraction?.status === "complete") {
        handleExtraction(responseText.extraction);
      }
    } catch (error: any) {
  ...
  return (
    <>
      ...
      {showAIChat && (
        ...
        >
          <Card
            className="w-full max-w-2xl h-[600px] flex flex-col border-2 border-purple-200"
